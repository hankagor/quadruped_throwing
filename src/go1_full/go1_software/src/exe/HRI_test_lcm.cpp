/*****************************************************************
 Test HRI interface with LCM on hardware. 

 Features (desired or already implemented):
    - "rear up" on back legs
    - balance on 3 legs for handshake 
...
public:
    ...
    int time_vector[3] = { 5, 145, 150 }; // times to switch modes in seconds
    // int last_time = 12;
    ...
    if( motiontime >= 0){
        ...
        if( motiontime >= 10 && motiontime < 3000 ) { //time_vector[0]*1000){
            ...
            for (int i=0; i<12; i++){
                ...
                qDes[i] = jointLinearInterpolation(init_joint_pos[i], hri.default_joint_pos[i], rate);
                dqDes[i] = 0;
                tauDes[i] = 0;
            }
        }
        ...
        if( motiontime >= time_vector[0]*1000 &&  motiontime < time_vector[0]*1000 + hri.full_opt_N){
            // all control logic
            hri.setLegControllerCommandsFromTrajIndex(motiontime - time_vector[0]*1000);
        ...
        // move front legs to init pos
        // first record current pos for front only  
        if( motiontime > time_vector[0]*1000 + 1.5*hri.full_opt_N && 
                motiontime < time_vector[0]*1000 + 1.5*hri.full_opt_N + 10){
            for (int i=0; i < 6; i++){
                init_joint_pos[i] = lowState.motorState[i].q;
            }
            rate_count = 0;
        ...
        if( motiontime > time_vector[0]*1000 + 1.5*hri.full_opt_N && 
                motiontime < time_vector[0]*1000 + 1.5*hri.full_opt_N + 1010){
            rate_count++;
            double rate = rate_count/1000.0;
            for (int i=0; i < 6; i++){
                qDes[i] = jointLinearInterpolation(init_joint_pos[i], hri.nom_front_feet_joint_pos[i], rate);
            }
        }

        //move front legs 
        if( motiontime > time_vector[0]*1000 + 2.6*hri.full_opt_N && 
            motiontime < (time_vector[1])*1000){
            // all control logic
            hri.setLegControllerCommandsFromOscillators();
            // set corresponding commands for FRONT LEGS ONLY 
            for (int i=0; i<6; i++){
                dqDes[i] = 0; Kp[i] = 0; Kd[i] = 1.5; 
        ...
        // now move to original pos in one second 
            ...
            rate_count++;
            double rate = rate_count/1300.0;  // one second 
            for (int i=0; i<12; i++){
                Kp[i] = 45; //5.0;
                Kd[i] = 1.5;
                qDes[i] = jointLinearInterpolation(init_joint_pos[i], hri.orig_joint_pos[i], rate);
                dqDes[i] = 0;
                tauDes[i] = 0;
            }
        }
